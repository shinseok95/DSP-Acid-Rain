...
private:
    ...
    list<string> serverword; // 서버에서 받아온 단어 리스트 (string)
    Font font;
    Text text;
    Player *player;
...
void Game::severAtk () { //임시 단어 리스트 관리
    ...
    while (temlist.size() != 10) { // 임시 단어들 리스트의 갯수가 10개 미만이면 x좌표 랜덤 지정해서 넣는다.
        int randomX = rand() % 560;
        cout << randomX << serverword.front() << endl;
...
void Game::update(Time elapsed){
    for (tlistIter = tlist.begin(); tlistIter != tlist.end(); ++tlistIter) {
        if (tlistIter->getPosition().y == 690) {
            tlist.erase(tlistIter);
            //HP바 줄어들음
        } else {
            tlistIter->move(0,elapsed.asSeconds()*30);
        }
    }
...
Game::Game(string name, string gradNum) {
    player = new Player(name, gradNum);
    font.loadFromFile("Nanum.ttf");
    //text.setPosition(10,20);
    //list<string> serverword = player->getServerWord();
    serverword.push_back("hello1");
    serverword.push_back("hello2");
    serverword.push_back("hello3");
    serverword.push_back("hello4");
    serverword.push_back("hello5");
    serverword.push_back("hello5");
    serverword.push_back("hello5");
    serverword.push_back("hello5");
    serverword.push_back("hello5");
